 ...
 * @return void
 */
function local_adminsettings_extend_navigation(global_navigation $navigation) {
    global $DB, $USER;
    if(is_siteadmin()){
        $sidebarnav = "Module Video Report | ".$CFG->wwwroot."/local/adminsettings/index.php";
        $menu = new custom_menu($sidebarnav, current_language());
        if ($menu->has_children()) {
            foreach ($menu->get_children() as $item) {
                adminsettings_custom_menu_item($item, 0, null, true);
            }
        }
    }
}
 ...
 * @return void
 */
function adminsettings_custom_menu_item(custom_menu_item $menunode, $parent, $pmasternode, $flatenabled) {
    global $PAGE, $CFG;
    ...
    if ($menunode->has_children()) {
        ...
        if ($parent > 0) {
            $masternode = $pmasternode->add(local_adminsettings_get_string($menunode->get_text()),
                                            $url, navigation_node::TYPE_CONTAINER);
            $masternode->title($menunode->get_title());
        } else {
            $masternode = $PAGE->navigation->add(local_adminsettings_get_string($menunode->get_text()),
                                            $url, navigation_node::TYPE_CONTAINER);
            $masternode->title($menunode->get_title());
            if ($flatenabled) {
        ...
        foreach ($menunode->get_children() as $menunode) {
            adminsettings_custom_menu_item($menunode, $submenucount, $masternode, $flatenabled);
        }
        // print_r($menunode);
        // echo "</pre>";
        // die;
    ...
    // print_r($parent);
        if ($parent) {
            $childnode = $pmasternode->add(local_adminsettings_get_string($menunode->get_text()),
 ...
 * @return string
 */
function local_adminsettings_get_string($string) {
    $title = $string;
